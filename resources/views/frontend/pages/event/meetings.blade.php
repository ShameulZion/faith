    ...
    <section class="page-banner">
        ...
        <div class="auto-container">
          ...
          <ul class="bread-crumb clearfix">
            ...
            <li>Current Event</li>
            <li class="active">Our Meetings</li>
          </ul>
        </div>
      </section>
      <!--End Banner Section -->
            ...
            <div class="row clearfix">
                ...
                <div class="event-block-three col-lg-4 col-md-6 col-sm-12">
                    <div class="inner-box wow fadeInUp animated" data-wow-delay="0ms" style="visibility: visible; animation-delay: 0ms; animation-name: fadeInUp" >
                        <div class="image-box">
                            <figure class="image">
                                <a href="{{ url($meeting->slug) }}">
                                    @foreach($meeting->media as $attachment)
                                        @if($attachment->mime_type == "video/mp4")
                                            <video  height="268px" src="{{ $meeting->getFirstMediaUrl('attach') }}" controls></video>
                                        @elseif($attachment->mime_type == "application/pdf")
                                            <iframe width="100%" height="268px" src="{{ $meeting->getFirstMediaUrl('attach') }}" frameborder="0"></iframe>
                                        @else
                                            <img width="100%" height="268px" class="lazy-image loaded" src="{{ $meeting->getFirstMediaUrl('attach') }}" alt="{{ $meeting->title }}" data-was-processed="true"/>
                                        @endif
                                    @endforeach
                                </a>
                            </figure>
                        ...
                        <div class="lower-content">
                            <h3><a href="{{ url($meeting->slug) }}">{{ $meeting->title }}</a></h3>
                            @if(!empty($meeting->content))
                                <div>{!!  substr(strip_tags($meeting->content), 0, 150) !!}</div>
                            @endif
                            <div class="link-box mt-2">
                                <a href="{{ url($meeting->slug) }}" class="theme-btn btn-style-two"><span class="btn-title">Read More</span></a>
                            </div>
                        </div>
                    </div>
                </div>
