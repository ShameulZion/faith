            ...
            <form role="form" id="NewsFrom" method="POST" action="{{ isset($event) ? route('admin.event.update',$event->id) : route('admin.event.store') }}" enctype="multipart/form-data">
                ...
                <div class="row">
                        ...
                        <div class="main-card mb-3 card">
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="content" id="editor1" placeholder="Description" class="form-control @error('content') is-invalid @enderror">{{ $event->content ?? old('content')  }}</textarea>
                                    @error('content')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                    ...
                    <div class="col-md-4">
                        <div class="main-card mb-3 card">
                            <div class="card-body">
                                <h5 class="card-title">Image, Video and Status</h5>
                                
                                <div class="form-group">
                                    <label for="">Image Or Video <small>(Max size: 10mb)</small></label>
                                    <input type="file" class="dropify @error('attach') is-invalid @enderror" name="attach"  data-default-file="{{ isset($event) ? $event->getFirstMediaUrl('attach') : old('attach')  }}" />
                                    @error('attach')
